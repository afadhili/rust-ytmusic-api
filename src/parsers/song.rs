...
impl SongParser {
    pub fn parse(data: &Value) -> SongFull {
        SongFull {
            ...
            formats: traverse_list(data, &["streamingData", "formats"]).into_iter().cloned().collect(),
            adaptive_formats: traverse_list(data, &["streamingData", "adaptiveFormats"]).into_iter().cloned().collect(),
        }
    }
    ...
    pub fn parse_search_result(item: &Value) -> SongDetailed {
        ...
        let title = flat.get(0).copied().or(columns.get(0).copied()).unwrap_or(item);
    ...
    pub fn parse_artist_top_song(item: &Value, artist: ArtistBasic) -> SongDetailed {
        ...
        SongDetailed {
            ...
            name: traverse_string(title, &["text"]),
            artist,
            album: Some(AlbumBasic { name: traverse_string(album, &["text"]), album_id: traverse_string(album, &["browseId"]) }),
            duration: None,
            thumbnails: Parser::parse_thumbnails(item, &["thumbnails"]),
        }
    }

    pub fn parse_album_song(item: &Value, artist: ArtistBasic, album: AlbumBasic, thumbnails: Vec<ThumbnailFull>) -> SongDetailed {
