...
impl VideoParser {
    pub fn parse(data: &Value) -> VideoFull {
        VideoFull {
            ...
            artist: ArtistBasic {
                artist_id: Some(traverse_string(data, &["videoDetails", "channelId"])).filter(|s| !s.is_empty()),
                name: traverse_string(data, &["author"]),
            },
            duration: traverse_string(data, &["videoDetails", "lengthSeconds"]).parse().unwrap_or(0),
            thumbnails: Parser::parse_thumbnails(data, &["videoDetails", "thumbnails"]),
            unlisted: traverse_list(data, &["unlisted"]).first().and_then(|v| v.as_bool()).unwrap_or(false),
            family_safe: traverse_list(data, &["familySafe"]).first().and_then(|v| v.as_bool()).unwrap_or(false),
            paid: traverse_list(data, &["paid"]).first().and_then(|v| v.as_bool()).unwrap_or(false),
    ...
    pub fn parse_search_result(item: &Value) -> VideoDetailed {
        ...
        let artist = flat.iter().copied().find(|v| is_artist(v)).or_else(|| flat.get(1).copied()).unwrap_or(item);
        ...
        VideoDetailed {
            ...
            artist: ArtistBasic { artist_id: Some(traverse_string(artist, &["browseId"])).filter(|s| !s.is_empty()), name: traverse_string(artist, &["text"]) },
            duration: Parser::parse_duration(duration.and_then(|d| d.get("text")).and_then(Value::as_str)),
    ...
    pub fn parse_playlist_video(item: &Value) -> Option<VideoDetailed> {
        ...
        let flat = flex.iter().flat_map(|v| match v { Value::Array(a) => a.iter().collect::<Vec<_>>(), _ => vec![*v] }).collect::<Vec<_>>();
        let title = flat.iter().copied().find(|v| is_title(v)).or_else(|| flat.get(0).copied()).unwrap_or(item);
        let artist = flat.iter().copied().find(|v| is_artist(v)).or_else(|| flat.get(1).copied()).unwrap_or(item);
        ...
        if video_id.is_empty() {
            if let Some(url) = Parser::parse_thumbnails(item, &["thumbnails"]).first().map(|t| t.url.clone()) {
        ...
        Some(VideoDetailed {
            video_id,
            name: traverse_string(title, &["text"]),
            artist: ArtistBasic { name: traverse_string(artist, &["text"]), artist_id: Some(traverse_string(artist, &["browseId"])).filter(|s| !s.is_empty()) },
            duration: Parser::parse_duration(duration.and_then(|d| d.get("text")).and_then(Value::as_str)),
            thumbnails: Parser::parse_thumbnails(item, &["thumbnails"]),
        })
    }
