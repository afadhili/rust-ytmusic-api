...
use crate::parsers::{
    album::AlbumParser, artist::ArtistParser, common::Parser, playlist::PlaylistParser,
    search::SearchParser, song::SongParser, video::VideoParser,
};
use crate::types::{
    AlbumDetailed, AlbumFull, ArtistDetailed, ArtistFull, HomeSection, PlaylistDetailed,
    PlaylistFull, SearchResult, SongDetailed, SongFull, UpNextDetails, VideoDetailed, VideoFull,
};
...
impl MusicClient {
    ...
    pub async fn init_with_options(
        ...
        if let Some(gl) = options.gl {
            self.config.insert("GL".to_string(), Value::String(gl));
        }
        if let Some(hl) = options.hl {
            self.config.insert("HL".to_string(), Value::String(hl));
        }
    ...
    async fn construct_request(
        ...
        let params = build_query_string(&query, &api_key);
        ...
        fn build_query_string(query: &HashMap<String, String>, api_key: &str) -> String {
    ...
    pub async fn raw_search(&self, query: &str) -> Result<Value, ClientError> {
        self.construct_request(
            "search",
            json!({ "query": query, "params": Value::Null }),
            HashMap::new(),
        )
        .await
    }
    ...
    pub async fn find(&self, query: &str) -> Result<Vec<SearchResult>, ClientError> {
        ...
        results.extend(
            self.find_songs(query)
                .await?
                .into_iter()
                .map(SearchResult::Song),
        );
        results.extend(
            self.find_videos(query)
                .await?
                .into_iter()
                .map(SearchResult::Video),
        );
        results.extend(
            self.find_artists(query)
                .await?
                .into_iter()
                .map(SearchResult::Artist),
        );
        results.extend(
            self.find_albums(query)
                .await?
                .into_iter()
                .map(SearchResult::Album),
        );
        results.extend(
            self.find_playlists(query)
                .await?
                .into_iter()
    ...
    pub async fn find_songs(&self, query: &str) -> Result<Vec<SongDetailed>, ClientError> {
        self.find_songs_with_filter(query, "Eg-KAQwIARAAGAAgACgAMABqChAEEAMQCRAFEAo%3D")
            .await
    }
    ...
    pub async fn find_videos(&self, query: &str) -> Result<Vec<VideoDetailed>, ClientError> {
        let data = self
            ...
            .await?;
        Ok(traverse_list(&data, &["musicResponsiveListItemRenderer"])
            .into_iter()
            .map(VideoParser::parse_search_result)
            .collect())
    }
    ...
    pub async fn find_artists(&self, query: &str) -> Result<Vec<ArtistDetailed>, ClientError> {
        let data = self
            .construct_request(
                "search",
                json!({ "query": query, "params": "Eg-KAQwIABAAGAAgASgAMABqChAEEAMQCRAFEAo%3D" }),
                HashMap::new(),
            )
            .await?;
        Ok(traverse_list(&data, &["musicResponsiveListItemRenderer"])
            .into_iter()
            .map(ArtistParser::parse_search_result)
            .collect())
    }
    ...
    pub async fn find_albums(&self, query: &str) -> Result<Vec<AlbumDetailed>, ClientError> {
        let data = self
            .construct_request(
                "search",
                json!({ "query": query, "params": "Eg-KAQwIABAAGAEgACgAMABqChAEEAMQCRAFEAo%3D" }),
                HashMap::new(),
            )
            .await?;
        Ok(traverse_list(&data, &["musicResponsiveListItemRenderer"])
            .into_iter()
            .map(AlbumParser::parse_search_result)
            .collect())
    }
    ...
    pub async fn find_playlists(&self, query: &str) -> Result<Vec<PlaylistDetailed>, ClientError> {
        let data = self
            .construct_request(
                "search",
                json!({ "query": query, "params": "Eg-KAQwIABAAGAAgACgBMABqChAEEAMQCRAFEAo%3D" }),
                HashMap::new(),
            )
            .await?;
        Ok(traverse_list(&data, &["musicResponsiveListItemRenderer"])
            .into_iter()
            .map(PlaylistParser::parse_search_result)
            .collect())
    }

    async fn find_songs_with_filter(
        &self,
        query: &str,
        params: &str,
    ) -> Result<Vec<SongDetailed>, ClientError> {
        let data = self
            .construct_request(
                "search",
        ...
        Ok(traverse_list(&data, &["musicResponsiveListItemRenderer"])
            .into_iter()
            .map(SongParser::parse_search_result)
            .collect())
    }
    ...
    pub async fn fetch_song(&self, video_id: &str) -> Result<SongFull, ClientError> {
        ...
        let data = self
            .construct_request("player", json!({ "videoId": video_id }), HashMap::new())
            .await?;
        let song = SongParser::parse(&data);
        if song.video_id != video_id {
            return Err(ClientError::InvalidVideoId);
        }
        Ok(song)
    }
    ...
    pub async fn fetch_watch_queue(
        &self,
        video_id: &str,
    ) -> Result<Vec<UpNextDetails>, ClientError> {
        validate_video_id(video_id)?;
        ...
        Ok(contents
            ...
            .filter_map(|item| {
                ...
                Some(UpNextDetails {
                    ...
                    duration: Parser::parse_duration(Some(&traverse_string(
                        renderer,
                        &["lengthText", "runs", "text"],
                    ))),
    ...
    pub async fn fetch_lyrics(&self, video_id: &str) -> Result<Option<Vec<String>>, ClientError> {
        validate_video_id(video_id)?;
        let data = self
            .construct_request("next", json!({ "videoId": video_id }), HashMap::new())
            .await?;
        let tabs = traverse_list(&data, &["tabs", "tabRenderer"]);
        let browse_id = tabs
            .get(1)
            .map(|v| traverse_string(v, &["browseId"]))
            .unwrap_or_default();
        if browse_id.is_empty() {
            return Ok(None);
        }
        let lyrics_data = self
            .construct_request("browse", json!({ "browseId": browse_id }), HashMap::new())
            .await?;
    ...
    pub async fn fetch_artist(&self, artist_id: &str) -> Result<ArtistFull, ClientError> {
        let data = self
            .construct_request("browse", json!({ "browseId": artist_id }), HashMap::new())
            .await?;
    ...
    pub async fn fetch_artist_songs(
        ...
        let artist_data = self
            ...
            .await?;
        let browse_token =
            traverse_string(&artist_data, &["musicShelfRenderer", "title", "browseId"]);
        if browse_token.is_empty() {
            return Ok(vec![]);
        }
        ...
        let songs_data = self
            .construct_request(
                "browse",
                json!({ "browseId": browse_token }),
                HashMap::new(),
            )
            .await?;
        let mut songs: Vec<SongDetailed> =
    ...
    pub async fn fetch_artist_albums(
        &self,
        artist_id: &str,
    ) -> Result<Vec<AlbumDetailed>, ClientError> {
        let artist_data = self
            .construct_request("browse", json!({ "browseId": artist_id }), HashMap::new())
            .await?;
        let artist_albums_data = traverse_list(&artist_data, &["musicCarouselShelfRenderer"])
            .first()
            .copied()
            .ok_or(ClientError::InvalidResponse)?;
        let browse_body =
            traverse_first(artist_albums_data, &["moreContentButton", "browseEndpoint"])
                .cloned()
                .unwrap_or_else(|| json!({}));
        let albums_data = self
            .construct_request("browse", browse_body, HashMap::new())
            .await?;
        let artist = crate::types::ArtistBasic {
            artist_id: Some(artist_id.to_string()),
            name: traverse_string(&albums_data, &["header", "runs", "text"]),
        };
        Ok(traverse_list(&albums_data, &["musicTwoRowItemRenderer"])
            .into_iter()
            .map(|item| AlbumParser::parse_artist_album(item, artist.clone()))
            .collect())
    }

    pub async fn fetch_album(&self, album_id: &str) -> Result<AlbumFull, ClientError> {
        let data = self
            .construct_request("browse", json!({ "browseId": album_id }), HashMap::new())
            .await?;
    ...
    pub async fn fetch_playlist_tracks(
        &self,
        playlist_id: &str,
    ) -> Result<Vec<VideoDetailed>, ClientError> {
        let browse_id = normalize_playlist_id(playlist_id);
        let playlist_data = self
            .construct_request("browse", json!({ "browseId": browse_id }), HashMap::new())
            .await?;

        let mut videos: Vec<VideoDetailed> = traverse_list(
            &playlist_data,
            &[
    ...
    pub async fn fetch_home(&self) -> Result<Vec<HomeSection>, ClientError> {
        let data = self
            .construct_request(
                "browse",
                json!({ "browseId": FE_MUSIC_HOME }),
                HashMap::new(),
            )
            .await?;

        let mut sections: Vec<HomeSection> =
            traverse_list(&data, &["sectionListRenderer", "contents"])
                .into_iter()
...
fn validate_video_id(video_id: &str) -> Result<(), ClientError> {
    ...
    if valid {
        ...
        Err(ClientError::InvalidVideoId)
    }
}
